...
async def check_user_history(event):
    try:
        ...
        get_user_id = event.message.raw_text.split()
        user_id_validation = len(get_user_id)
        valid_lenght = 2
        if user_id_validation == valid_lenght:
            user_id = int(get_user_id[1])
            ...
            async with crowgram.conversation(bot) as start_conversation:
                await start_conversation.send_message(f"{user_id}")
