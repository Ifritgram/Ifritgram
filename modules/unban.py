...
async def rununban(event):
    ...
    try:
        await event.client.edit_permissions(messagelocation, targetuser, view_messages=True)
        await event.client.send_message(messagelocation, f"<a href='tg://user?id={targetuser}'>{targetdetails.users[0].first_name}</a> Has Been Successfully Unbanned")
    except ValueError:
