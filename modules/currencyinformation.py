...
async def runci(event):
    ...
    try:
        if selectoption[1] == "s":
            ...
            currencyname = getinformationaboutcurrency.get_currency_name(f"{getcurrencyname}")
            currencysymbol = getinformationaboutcurrency.get_symbol(f"{getcurrencyname}")
            await event.client.send_message(messagelocation, f"Currency Name: {currencyname}\nCurrency Symbol: {currencysymbol}")
        elif selectoption[1] == "cr":
            getcurrencyrates = CurrencyRates()
            rates = getcurrencyrates.get_rate(f"{selectoption[2]}", f"{selectoption[3]}")
            ...
            await event.client.send_message(messagelocation, f"{price} {selectoption[2]}")
        else:
