...
async def runrmf(event):
    ...
    try:
        gather(*[client.delete_messages(messagelocation, id) for id in [msg.id for msg in await client.get_messages(messagelocation, limit=int(messageslimit[1])) if msg]])
    except:
