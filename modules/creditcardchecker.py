...
async def runn(event):
    ...
    try:
        if CreditCardChecker(f"{creditcardnumberis[0]}").valid():
            await event.client.send_message(messagelocation, f"Congratulations, {creditcardnumberis[0]} Is Valid.")
        else:
            await event.client.send_message(messagelocation, f"Sorry, {creditcardnumberis[0]} Is Invalid.")
    except:
        pass