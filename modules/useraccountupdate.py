...
async def runuau(event):
    ...
    try:
        if selectoption[1] == "nc":
            await event.client(UpdateProfileRequest(first_name=f"{getupdate.message}"))
            await event.edit("Name Changed")
        elif selectoption[1] == "bc":
            await event.client(UpdateProfileRequest(about=f"{getupdate.message}"))
            ...
            await event.client(UploadProfilePhotoRequest(await event.client.upload_file(f"{downloadtargetcontent}")))
